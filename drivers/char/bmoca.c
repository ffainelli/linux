...
static int moca_parse_dt_node(struct moca_priv_data *priv)
{
	...
	priv->clk = devm_clk_get_optional(&pdev->dev, "sw_moca");
	if (IS_ERR(priv->clk)) {
		dev_err(&pdev->dev,
	...
	priv->cpu_clk = devm_clk_get_optional(&pdev->dev, "sw_moca_cpu");
	...
	priv->phy_clk = devm_clk_get_optional(&pdev->dev, "sw_moca_phy");
	...
	ret = of_get_mac_address(of_node, macaddr);
	if (ret < 0) {
