...
static void scu_pic_wdt_timerfunc(struct timer_list *t)
{
	struct scu_pic_data *data = from_timer(data, t, wdt_timer);
	struct watchdog_device *wdev = &data->wdt_dev;
...
static int scu_pic_wdt_init(struct i2c_client *client)
{
	...
	timer_setup(&data->wdt_timer, scu_pic_wdt_timerfunc, 0);
